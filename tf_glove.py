...
class GloVeModel():
    def __init__(self, embedding_size, context_size, max_vocab_size=100000, min_occurrences=1,
                 scaling_factor=3.0/4.0, cooccurrence_cap=100, batch_size=128, learning_rate=0.05):
    ...
    def __fit_to_corpus(self, corpus, vocab_size, min_occurrences, left_size, right_size):
        ...
        for word, i in self.__word_to_id.items():
            mapping[word]=i
            print (word, i)
    ...
    def train(self, num_epochs, log_dir=None, summary_batch_interval=1000,
        ...
        with tf.Session(graph=self.__graph, config=tf.ConfigProto(gpu_options=gpu_options)) as session:
...
def get_mapping(model):
    for word in mapping.keys():
        print(word, model.embeddings[mapping[word]])
